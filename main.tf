...
resource "tls_private_key" "ec2_private_key" {
    ...
    provisioner "local-exec" {
      command = "echo '${tls_private_key.ec2_private_key.private_key_pem}' > ${terraform.workspace}/${var.key_name}.pem"
    }
}
...
resource "null_resource" "key-perm" {
    ...
    provisioner "local-exec" {
      command = "chmod 400 ${terraform.workspace}/${var.key_name}.pem"
    }
}